...
for atom, active_spaces in [(atom_c, {'singlet': as_c_singlet, 'triplet': as_c_triplet}),
    ...
    for basis_name in ['cc-pCVTZ']:
        ...
        for i, state in enumerate(output_s['excited_states_rasci']):
        ...
        for i, state in enumerate(output_t['excited_states_rasci']):
            print('Energy state {} ({}):  {: 18.12f} au'.format(i+1, state['multiplicity'], state['total_energy']))
            print(' Alpha  Beta   Amplitude')
            for j, conf in enumerate(state['configurations']):
                print('  {}  {} {:8.3f}'.format(conf['alpha'], conf['beta'], conf['amplitude']))
        ...
        for i in range(3):
            for j in range(3):
                coupling[3*i+j, 3*i+j] = output_s['excited_states_rasci'][i]['excitation_energy']*8065.54429 # eV to cm-1

        for i in range(6):
            coupling[i + 9, i + 9] = output_s['excited_states_rasci'][i+3]['excitation_energy']*8065.54429 # eV to cm-1
