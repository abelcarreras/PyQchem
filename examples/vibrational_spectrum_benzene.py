# Example of the calculation of the emission/absorption spectrum for benzene
...
molecule = get_geometry_from_pubchem('benzene')

basis_set = '6-311G**'
n_state = 1  # excited state number (target)
temperature = 40  # 100 # 40
plot_marcus = False
...
excitation_energy = (es_energy - gs_energy) * AU_TO_EV + exp_off_set
...
print((es_gs_energy - es_energy) * AU_TO_EV, (gs_es_energy - gs_energy)* AU_TO_EV)
...
min = transitions[0].energy_emission-sigma*5
max = transitions[-1].energy_absorption+sigma*5
...
if experiment:
    from pyqchem.units import EV_TO_NM

    # experimental data
    e_abs, int_abs = np.loadtxt('data/benzene_abs').T
    e_em, int_em =np.loadtxt('data/benzene_em').T

    # convert nm to eV
    e_em = EV_TO_NM/e_em
    e_abs = EV_TO_NM/e_abs

    # normalize spectrum
    int_abs /= np.trapezoid(int_abs, e_abs)
    int_em /= -np.trapezoid(int_em, e_em)

    plt.plot(e_abs, int_abs, color='gray', label='experiment')
    plt.plot(e_em, int_em, color='gray')
